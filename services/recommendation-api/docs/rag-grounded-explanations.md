...
설명 검토는 후보당 최대 1회 추가 호출한다. 질문이 있는 요청의 source rerank는 최대 8개 후보 단위로 batch 처리하므로 설명 단계의 통상 호출 수는 `ceil(후보 수/8) + 후보 수`이고, 재서술 검토가 필요한 카드만 후보당 최대 1회가 추가된다. planner와 후보 선택 rerank는 각각 별도 1회이며 공급자 호환 재시도는 별도일 수 있다. `LLM_MAX_CALLS_PER_RUN`과 API 시간 제한을 운영 환경에 맞게 설정해야 한다. `calls_succeeded`는 종전처럼 채택된 설명 카드 수이며 실제 HTTP 호출 횟수가 아니다.
...
후보 점수·등급·후보 간 순위는 변경하지 않는다. 토큰 검색은 의미가 비슷한 다른 표현을 놓칠 수 있고, LLM 재정렬의 실제 품질은 질문별 평가가 필요하다. 벡터 검색/전용 cross-encoder는 추가하지 않았다. source rerank는 후보 최대8개씩 batch 호출하므로 질문이 있는 설명 단계의 source-rerank 호출 수는 `ceil(후보 수/8)`이다. 후보별 설명 생성과 선택적 검증 호출은 후보 단위로 유지한다.
