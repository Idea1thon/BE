...
pooled = pd.read_csv(os.path.join(OUT, "rent_sales_correlation_pooled.csv"), index_col=0)
by_biz = pd.read_csv(os.path.join(OUT, "rent_sales_correlation_by_industry.csv"), index_col=0)

draw_heatmap(
    pooled,
    "임대료·공실률·수익률 x 추정매출 지표 상관관계",
    "겹치는 63개 지역, 2021Q1~2025Q4(20개 분기), 10개 외식업종 통합 (n=12,600) · log1p Pearson r",
    os.path.join(FIG_DIR, "rent_sales_correlation_pooled.png"),
    vmax=0.3,
)

draw_heatmap(
    by_biz,
    "업종별 임대료 x 추정매출 지표 상관관계",
    "겹치는 63개 지역, 2021Q1~2025Q4(20개 분기) · log1p Pearson r (임대료 기준)",
    os.path.join(FIG_DIR, "rent_sales_correlation_by_industry.png"),
    vmax=0.3,
)
