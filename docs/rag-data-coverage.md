...
현재 SQL 허용 차원은 매출·점포·유동·상권변화와 임대가격지수·공실률·직장인구다. 질문에서 선택한 차원만 최종 후보 상권(최대50개)과 선택 행정동 코드에서 공간 단위를 구분해 읽는다. 입력 지역 DB의 코드는 추천 서비스까지 유지하며, 행정동은 후보 상권이 없어도 독립 검색한다. 임대·공실은 유일한 적격 R-ONE 권역의 최신 소규모상가 관측, 직장인구는 요청 분기 이하 최신 관측을 사용하며, 기존 매출·점포·유동·변화는 요청 분기를 사용한다. 복수 임대 권역·테이블 미적재·조회 오류는 임의 값이나 0으로 대체하지 않는다.

출처 목록 전체는 감사용으로 보존하고 설명 모델에는 주제 가산점·어휘 관련도·핵심어 대표 선택을 적용한 출처 총 최대32개와 필수 경고만 제공한다. 정규화 SQL을 출처가 붙은 배경 문장으로 서버 렌더링하며, 선택 후보 사이의 동일 지표 관측 차이는 별도 비교표와 카드에 연결한다. 개별 매물 월세·실제 점심 방문량은 이번 검색으로 확인되지 않는다. 자세한 응답·기간·비교 불가 계약은 `services/recommendation-api/docs/rag-grounded-explanations.md`를 따른다.
