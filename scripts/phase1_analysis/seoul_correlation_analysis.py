"""서울 전체 상권 대상: 유동인구/상주인구/직장인구 vs 업종별(10개 독립 코드) 매출 상관관계 분석.

scripts/songpa_correlation_analysis.py를 송파구 한정 없이 서울 전체 상권으로 일반화한 버전.
(원본은 output/songpa/에 그대로 남겨 송파구 검증 결과와 비교 가능하게 유지)

방법 메모(가정, songpa 버전과 동일):
...
- 업종에 매출이 아예 없는(=그 업종 점포가 없는) 상권은 0으로 채운다 (결측이 아니라 "그 업종 수요가
  0으로 관측됨"으로 취급).
- 상권 필터링 없음(서울 전체 1,650개 상권 전부 대상) — songpa_membership.py 같은 겹침 크로스워크
  필터가 필요 없음(자치구 경계를 따질 필요가 없으므로).
"""
import os
...
def load_sales():
    ...
    df = df[(df["기준_년분기_코드"] == QUARTER) & df["서비스_업종_코드"].isin(INDUSTRY_CODES)]
...
def plot_heatmap(corr):
    ...
    ax.set_title("서울 전체 상권: 인구지표 × 업종별 매출 상관계수 (log1p, Pearson)", color=INK_PRIMARY, fontsize=13, pad=14)
    cbar = fig.colorbar(im, ax=ax, shrink=0.8)
    cbar.ax.tick_params(colors=INK_MUTED)
    cbar.outline.set_visible(False)

    fig.tight_layout()
    out = os.path.join(FIG_DIR, "seoul_population_sales_corr_heatmap.png")
...
def scatter_panel(ax, x, y, xlabel, ylabel, corr_value):
    ...
    ax.scatter(x, y, color=BLUE, alpha=0.4, s=18, edgecolor=SURFACE, linewidth=0.4, zorder=2)
...
def plot_scatter_examples(log_df, corr):
    ...
    fig.suptitle("서울 전체 상권: 인구지표 vs 업종별 매출 산점도 (예시 4쌍)", color=INK_PRIMARY, fontsize=13)
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    out = os.path.join(FIG_DIR, "seoul_population_sales_scatter_examples.png")
    fig.savefig(out, dpi=150, facecolor=SURFACE)
    plt.close(fig)
    print(f"{out} 저장")
...
def main():
    ...
    print(f"분석 대상 서울 전체 상권 수: {len(merged)}개 (인구 또는 매출 데이터가 1개 이상 존재)")

    log_df = np.log1p(merged)
    corr_full = log_df.corr(method="pearson")
    corr = corr_full.loc[POP_COLS, list(INDUSTRY_CODES.values())]

    corr_path = os.path.join(SEOUL_DIR, "seoul_population_sales_correlation.csv")
