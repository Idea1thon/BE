...
### 상업용 건물 모집단 (매물 공백의 부분 대체) — **Tier 1 이식 완료 2026-09-04**

매물 데이터가 없어도, 실재하는 상업용 건물의 좌표·용도·규모를 확보해 격자 합성좌표(`output/generated_evidence/`)를 대체한다.

| 출처 | 내용 | 접근 | 상태 |
| --- | --- | --- | --- |
| [국토교통부 GIS건물통합정보](https://www.data.go.kr/data/15083092/fileData.do) (15083092) | 건물 footprint 폴리곤 + 건축물대장 속성(주용도·층수·면적·건폐율·용적률·사용승인일·지번주소). EPSG:5186, dbf cp949. 아파트 이식과 원천 공유 | 무료 파일. 이미 보유 | **이식 완료** → `scripts/ingest_building_ledger.py` → `data/건축물대장/` → `context.commercial_building` (`--phase building`, `db/002_commercial_building.sql`). 서울 상업건물 115,141동(근생1·2/판매), 상권 1,622/1,650. **건물 주용도 기준** — 층별용도·전유부 호실·주차·공실·임대료 없음 |
| [국토교통부 건축HUB 건축물대장정보 서비스](https://www.data.go.kr/data/15134735/openapi.do) (15134735) | 표제부·층별개요·전유부·전유공용면적 등. **층별 용도·면적**, **구분상가 호실**, **부설주차대수** | 무료 API, `DATA_GO_KR_SERVICE_KEY` 공용, 개발계정 자동승인·일 10,000회 | **Tier 2 미착수**. 서울 법정동(~467) 순회, 파일럿 1개 자치구부터 |

→ **Tier 1은 "건물이 상업 용도"라는 사실이지 매물·공실·임대료가 아니다.** `synthetic_anchor` 격자 대체 seed와 상권별 상가 건물 규모 배경으로만 쓰고, F11(정확한 매물 주소 미제공)은 유지한다.
...
| ~~1~~ 완료 | 상업용 건물 모집단 (Tier 1) | GIS건물통합정보 15083092 → `data/건축물대장/` → `context.commercial_building` (`scripts/ingest_building_ledger.py`, `db/002_commercial_building.sql`). 서울 상업건물 115,141동 |
| 1 | 건물 층별용도·전유부 호실·주차 (Tier 2) | 건축HUB API 15134735 (`DATA_GO_KR_SERVICE_KEY`, 자동승인). 법정동 순회, 파일럿 1구부터 |
| — | 개별 매물 (임대조건·공실·링크) | 디스코·밸류맵·부동산플래닛 | **높음, 사람 승인** (제휴·크롤링 약관) |
