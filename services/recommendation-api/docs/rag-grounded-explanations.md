...
2. 질문의 명시된 지표를 공통 질문 계약으로 만들고, 기존 엔진이 선택한 최종 후보 상권을 읽기 전용 검색 대상으로 지정한다. 지역·업종·기간 정책은 서버가 결정한다. 결과에서 출처·지역·기간·단위가 확인되는 행만 `retrieval_evidence`로 정규화한다. 결측값을 0으로 바꾸지 않는다. 행정동 검색에는 선택 시군구 조건도 적용한다. DB 모드에서는 dataset-run stamp·TTL 캐시를 사용하고, 같은 실행의 중복 tool 요청은 동일 SQL을 memoize해 재조회하지 않는다.
