"""서울 전체 상권: 지금까지 따로 분석한 인구(유동/상주/직장)·아파트(소득)·점포·상권변화지표를
하나의 피처 테이블로 합쳐 업종별 매출을 다변량으로 설명하는 회귀분석.

scripts/songpa_multivariate_analysis.py를 송파구 한정 없이 서울 전체 1,650개 상권으로
일반화한 버전(원본은 output/songpa/에 그대로 남겨 송파구 검증 결과와 비교 가능하게 유지).

방법(가정, songpa 버전과 동일):
- sklearn/statsmodels 미설치 상태라 numpy로 표준화(z-score) 다중선형회귀를 직접 구현.
- 상권변화지표(gap_survive/gap_close)·인구·아파트는 업종 구분이 없는 상권 단위 값이라,
  그 상권의 업종 10개 행 모두에 동일한 값을 broadcast join한다.
...
def load_area_scalar(rel_path, value_col, quarters, out_name):
...
def build_panel(quarters):
    flow = load_area_scalar("길단위인구/서울시 상권분석서비스(길단위인구-상권).csv", "총_유동인구_수", quarters, "유동인구")
    resident = load_area_scalar("상주인구/서울시 상권분석서비스(상주인구-상권).csv", "총_상주인구_수", quarters, "상주인구")
    worker = load_area_scalar("직장인구/서울시 상권분석서비스(직장인구-상권).csv", "총_직장_인구_수", quarters, "직장인구")

    apt = pd.read_csv(os.path.join(DATA, "아파트/서울시 상권분석서비스(아파트-상권).csv"), encoding="cp949")
    apt = apt[apt["기준_년분기_코드"].isin(quarters)]
    ...
    ci = ci[ci["기준_년분기_코드"].isin(quarters)].copy()
    ...
    store = store[
        store["기준_년분기_코드"].isin(quarters) & store["서비스_업종_코드"].isin(INDUSTRY_CODES)
    ].copy()
    ...
    sales = sales[
        sales["기준_년분기_코드"].isin(quarters) & sales["서비스_업종_코드"].isin(INDUSTRY_CODES)
    ].copy()
...
def main():
    ...
    panel_csv = os.path.join(SEOUL_DIR, "seoul_multivariate_panel.csv")
    ...
    predictor_corr_csv = os.path.join(SEOUL_DIR, "seoul_multivariate_predictor_corr.csv")
    ...
    coef_csv = os.path.join(SEOUL_DIR, "seoul_multivariate_standardized_coef.csv")
    ...
    r2_csv = os.path.join(SEOUL_DIR, "seoul_multivariate_r2_compare.csv")
    ...
    ax.set_title("서울 전체: 업종별 표준화 다중회귀계수 (매출 설명, log1p)", color=INK_PRIMARY, fontsize=12, pad=12)
    cbar = fig.colorbar(im, ax=ax, shrink=0.8)
    cbar.ax.tick_params(colors=INK_MUTED)
    cbar.outline.set_visible(False)
    fig.tight_layout()
    out1 = os.path.join(FIG_DIR, "seoul_multivariate_coef_heatmap.png")
    ...
    ax.set_title("서울 전체: 다변량 회귀 R² vs 최고 단일지표 R² 비교", color=INK_PRIMARY, fontsize=12)
    ...
    fig.tight_layout()
    out2 = os.path.join(FIG_DIR, "seoul_multivariate_r2_compare.png")
    ...
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.set_title("서울 전체: 예측변수 간 상관계수 (다중공선성 점검)", color=INK_PRIMARY, fontsize=12, pad=12)
    cbar = fig.colorbar(im, ax=ax, shrink=0.8)
    cbar.ax.tick_params(colors=INK_MUTED)
    cbar.outline.set_visible(False)
    fig.tight_layout()
    out3 = os.path.join(FIG_DIR, "seoul_multivariate_predictor_corr.png")
