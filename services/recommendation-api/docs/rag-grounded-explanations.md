...
단어·한글 2글자 토큰 겹침으로 최대48개 근거를 추리고, 요청 내 짧은 ID(`E01` 등)로 매핑한다. LLM에는 상위 최대16개 ID만 관련도 순서로 선택하도록 요청한다. 서버는 유효한 ID의 순서를 보존하고 중복은 첫 항목만 사용하며, 알 수 없는 ID와 문자열이 아닌 값은 제외한다. 부족한 순위는 기존 토큰 관련도 순서로 보충한다. 유효한 ID가 하나도 없거나 응답 형식/호출이 실패한 경우에만 전체 토큰 순서로 복귀한다. 짧은 ID는 요청 내부에서만 사용하며 반환 출처 ID와 인용 계약은 바뀌지 않는다. 질문 주제에 맞는 일반 출처 최대16개와 필수 반대근거·결측·등급 요약만 설명에 전달한다. 후보 지표와 SQL 근거도 일반 출처 상한에 포함하며 무조건 재포함하지 않는다. 필수 경고가 많으면 전체 전달 수는16개보다 많을 수 있다. 전체 출처는 응답에 보존하고 선택 목록·정렬 방식은 `relevance_by_candidate`에 기록한다. 최종 문장도 같은 종류 안에서 관련 근거 순서로 정렬하고 인용 위치를 함께 갱신한다.
...
검색은 후보 정렬·limit 적용 뒤 수행한다. `target_areas=[]`는 검색 대상을 찾지 못했다는 뜻이며 전체 지역 검색으로 대체하지 않는다. 상권 최대50개를 조회하고 선택 구·동 범위를 함께 적용한다. 동 이름 메타데이터가 비어 있을 수 있어 동일 구·동 경계의 면적 교차로도 확인한다. sales/stores/flow/change는 요청 분기, 직장인구는 요청 분기를 넘지 않는 최신 유효 관측, 임대·공실은 기존 후보 엔진처럼 최신 유효 소규모상가 분기를 사용한다. 따라서 출처마다 기간이 다를 수 있다.

임대·공실은 `join_eligible`인 유일한 R-ONE 대리 권역만 조회한다. 복수 권역 매핑은 임의 선택하지 않으며 서울 전체 값으로 대체하지 않는다. `results[].availability`는 차원별 `available/missing/error`와 `rows_returned/no_rows/no_target_areas/table_unavailable/query_failed`를 구분한다. 행 존재와 개별 후보의 정상 관측 여부는 다를 수 있으므로 최종 비교표의 cell 상태도 확인해야 한다.

설명 후처리는 질문 관련 출처에 연결되지 않은 선택적 이유·배경 문장을 제거한다. 템플릿 복귀에도 같은 정책을 적용하며 필수 경고·결측은 유지한다. 검증된 SQL 행은 같은 후보 상권·업종인지 확인한 뒤 주제당 최대1개 배경 문장으로 직접 표시한다. 출처·기간·단위·실제 조사권역·대리지표 한계를 포함하며 `retrieval-*`를 인용한다. 같은 주제에 복수 값/기간/권역이 있으면 단일 값을 임의 선택하지 않고 미확인 사유를 표시한다. 이 서버 렌더링은 모델의 재서술 의미 검토와 별개이며 추가 LLM 호출이 없다.

`question_comparison`은 선택 후보의 지표값과 출처 ID, 결측/모호성, 기간·단위·지표·공간 범위를 비교한다. 같은 상권의 배경값은 후보 차이로 해석하지 않는다. 임대지수는 권역 간 절대 월세 비교에 사용하지 않는다. 원천 기간이 서로 다르면 차이를 계산하지 않는다. SQL 총매출/총유동은 후보의 점포당 매출/유동밀도와 섞지 않는다. 비교 가능한 쌍은 카드에도 주제당 첫1쌍씩 관측값 차이를 설명하고, 양측 원 관측을 담은 `comparison:*` 출처를 인용한다. 비교 불가 또는 limit1은 카드의 미확인 사유로 표시한다. 후보 등급·선택·순위는 그대로 유지한다.
