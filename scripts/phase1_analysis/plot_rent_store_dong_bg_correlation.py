...
# ---------- 행정동 ----------
pooled_dong = pd.read_csv(os.path.join(OUT, "rent_store_dong_correlation_pooled.csv"), index_col=0)
by_biz_dong = pd.read_csv(os.path.join(OUT, "rent_store_dong_correlation_by_industry.csv"), index_col=0)

draw_heatmap(
    pooled_dong,
    "임대료·공실률·수익률 x 점포 지표 상관관계 (행정동 단위)",
    "63개 지역의 소속 행정동(면적가중 다중소속), 2021Q1~2025Q4(20개 분기), 10개 외식업종 통합 (n=11,460) · log1p Pearson r",
    os.path.join(FIG_DIR, "rent_store_dong_correlation_pooled.png"),
    vmax=0.3,
)
draw_heatmap(
    by_biz_dong,
    "업종별 임대료 x 점포 지표 상관관계 (행정동 단위)",
    "63개 지역의 소속 행정동(면적가중 다중소속), 2021Q1~2025Q4(20개 분기) · log1p Pearson r (임대료 기준)",
    os.path.join(FIG_DIR, "rent_store_dong_correlation_by_industry.png"),
    vmax=0.4,
)

# ---------- 상권배후지 ----------
pooled_bg = pd.read_csv(os.path.join(OUT, "rent_store_bg_correlation_pooled.csv"), index_col=0)
by_biz_bg = pd.read_csv(os.path.join(OUT, "rent_store_bg_correlation_by_industry.csv"), index_col=0)

draw_heatmap(
    pooled_bg,
    "임대료·공실률·수익률 x 점포 지표 상관관계 (상권배후지 단위)",
    "63개 중 60개 지역의 소속 상권배후지(면적가중 다중소속), 2021Q1~2025Q4(20개 분기), 10개 외식업종 통합 (n=11,000) · log1p Pearson r",
    os.path.join(FIG_DIR, "rent_store_bg_correlation_pooled.png"),
    vmax=0.3,
)
draw_heatmap(
    by_biz_bg,
    "업종별 임대료 x 점포 지표 상관관계 (상권배후지 단위)",
    "63개 중 60개 지역의 소속 상권배후지(면적가중 다중소속), 2021Q1~2025Q4(20개 분기) · log1p Pearson r (임대료 기준)",
    os.path.join(FIG_DIR, "rent_store_bg_correlation_by_industry.png"),
    vmax=0.4,
)

# ---------- 3단위 비교 (임대료 x 점포, pooled / 지역x분기 독립표본) ----------
pooled_cmp = pd.read_csv(os.path.join(OUT, "rent_store_unit_comparison_pooled.csv"), index_col=0)
rq_cmp = pd.read_csv(os.path.join(OUT, "rent_store_unit_comparison_region_quarter.csv"), index_col=0)

draw_unit_comparison(
    pooled_cmp,
    "임대료 x 점포 상관계수, 공간 단위별 비교 (풀링)",
    "지역×분기×업종 패널 · log1p Pearson r · 상권=12,600행/행정동=12,600행/상권배후지=12,000행",
    os.path.join(FIG_DIR, "rent_store_unit_comparison_pooled.png"),
    n_labels={"상권(11-2)": "n≈11,460", "행정동": "n=11,460", "상권배후지": "n=11,000"},
)
draw_unit_comparison(
    rq_cmp,
    "임대료 x 점포 상관계수, 공간 단위별 비교 (지역x분기 독립표본)",
    "업종 10개 합산 후 지역×분기 단위 · log1p Pearson r",
    os.path.join(FIG_DIR, "rent_store_unit_comparison_region_quarter.png"),
    n_labels={"상권(11-2, n=1,260)": "n=1,260", "행정동": "n=1,260", "상권배후지": "n=1,200"},
)
