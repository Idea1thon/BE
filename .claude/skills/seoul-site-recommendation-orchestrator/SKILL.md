...
| --- | --- | --- | --- | --- |
| location-data-auditor | `.claude/agents/location-data-auditor.md` | inherit (부모 세션) | 정적 데이터 감사 | `artifacts/10-analysis/data-inventory.md`, `data-usage-classification.md` |
| location-input-contract-builder | `.claude/agents/location-input-contract-builder.md` | inherit (부모 세션) | UI·API 입력 계약 | `artifacts/10-analysis/input-and-condition-contract.md` |
| candidate-method-designer | `.claude/agents/candidate-method-designer.md` | inherit (부모 세션) | 후보 방법·상충 해소 | `artifacts/20-method/candidate-selection-spec.md` |
| rag-evidence-builder | `.claude/agents/rag-evidence-builder.md` | inherit (부모 세션) | Evidence JSON·설명 계약 | `artifacts/20-method/rag-evidence-schema.json` |
| recommendation-qa-reviewer | `.claude/agents/recommendation-qa-reviewer.md` | inherit (부모 세션) | 독립 반려 우선 QA | `artifacts/30-review/recommendation-quality-review.md` |

`TeamCreate` 시 위 Agent 파일의 역할을 팀 생성 지시에 그대로 사용한다. 모델은 각 Agent 파일의 `model: inherit`에 따라 부모 세션 모델을 상속한다(2026-09-02 사용자 요청 — 이전엔 gpt-5.6-* 개별 지정).
