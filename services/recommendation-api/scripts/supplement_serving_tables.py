...
* ``location.area_store_totals``: ``store_quarter``(대상 10개 업종)와
  분리된 전 업종 지역 배경값
* ``context.commercial_building``: 건축물대장 파생 상업용 건물 seed
* ``context.anchor_snapshot``·``context.news_manifest``·``context.rent_index``·
  ``context.naver_seasonality``: DB 서빙 경량 맥락
...
def main() -> int:
    ...
    print(f"rent_index: source rows {load_rent_index(source_ids)}")
    ...
    _run_sql(
        ...
        f"{_sql_literal('serving table supplement: area_store_totals, anchor/context snapshots, commercial_building')});"
    )
