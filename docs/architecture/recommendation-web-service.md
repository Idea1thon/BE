...
1. 사용자가 웹 폼(서울특별시 / 자치구 / (선택)행정동 + 업종 1개 + 특별조건 텍스트)을 제출하면 중간 백엔드가 요청을 접수하고,
...
- 동기 요청 경로: UI → 중간 백엔드(사용자 입력 처리) → FastAPI REST → 입력 정규화·계약 검증 → 후보 선정 엔진 → 근거 조립 → RAG 설명 → 중간 백엔드 응답
...
flowchart TB
    subgraph actors[외부 액터]
      ...
      BFF[중간 백엔드<br/>UI 입력 처리 · request_id]
    ...
    subgraph sys[입지 추천 파이프라인 서비스 경계]
      WEB[FastAPI 추천 파이프라인 REST]
    ...
    U -->|폼 입력 / 결과 표시| BFF
    BFF -->|지역·업종·특별조건 전달| WEB
    WEB -->|후보/Evidence/설명 반환| BFF
    WEB --> ENG
    ENG --> STORE
...
- **현재 저장소의 FastAPI 경계 안**: 파이프라인 REST 어댑터, 후보/근거/RAG 오케스트레이션, 읽기 저장소, 배치 파이프라인, raw 스냅샷 아카이브.
- **별도 시스템**: 웹 UI와 UI-facing 중간 백엔드(BFF). BFF가 `request_id`와 원문 사용자 요청을 관리한다.
- **경계 밖**: 모든 원천 데이터 제공기관, 지오코딩/검색 API, LLM API, 위험 사이렌 파이프라인(미구현).
- 사용자는 원천 데이터·LLM·FastAPI와 직접 통신하지 않는다. 중간 백엔드만 FastAPI를 호출하고, FastAPI 요청 경로만 LLM을 접촉한다.
...
| C2 | FastAPI 파이프라인 REST 경계 | 중간 백엔드 요청 수신, 형식 검증, `request_id` 전달, 파이프라인 실행, 응답 반환 | backend contract | pipeline response | 실행 아티팩트 경로(파생) | `api/main.py` |
| C3 | 입력 정규화·계약 검증기 | 지역명↔`data/영역/행정동` 코드 매핑, 업종 코드 검증, 평↔m² 변환, 요청 DTO 확정 | backend request | `ResolvedInput` 또는 `ConfirmationRequired` | 없음 | `input-and-condition-contract.md`, `scripts/make_gu_dong_lists.py` |
...
sequenceDiagram
    ...
    participant BFF as 중간 백엔드
    participant API as C2 FastAPI REST
    ...
    U->>BFF: 폼 입력(지역·업종·특별조건)
    BFF->>API: POST /internal/recommendations (backend contract)
    API->>API: 형식 검증 · request_id 태깅 · 응답캐시 조회
    alt 캐시 히트 (동일 요청 + 동일 data_version)
            API-->>BFF: 캐시된 RecommendationResponse
    else 캐시 미스
        ...
        alt 필수값 누락 or 애매/충돌
            ...
            API->>API: 응답 캐시 저장
            API-->>BFF: 200 RecommendationResponse
        end
    end
    BFF-->>U: 결과 표시
```
...
### 7.1 FastAPI REST API (중간 백엔드 전용)

| 메서드 · 경로 | 목적 | 요청 | 응답 | 계약 |
| --- | --- | --- | --- | --- |
| `GET /api/regions?sigungu=` | 중간 백엔드의 자치구·행정동 선택지 조회 | 쿼리 | `{sigungu[], dong[]}` | `data/영역/행정동`, `dong_lists/` |
| `GET /api/industries` | 중간 백엔드의 업종 선택지 조회 | — | `[{code, name}]` (CS100001~CS100010) | `AGENTS.md` 업종 계약 |
| `POST /internal/recommendations` | **핵심**: 추천 파이프라인 실행 | backend contract(§7.3) | `RecommendationResponse`(§7.4) | `candidate-selection-spec.md`, `rag-evidence-schema.json` |
| `POST /api/recommendations` | 기존 호출 호환 alias | backend contract(§7.3) | `RecommendationResponse`(§7.4) | 동일 |
| `GET /healthz` · `GET /readyz` | 라이브니스·레디니스(번들 로드 완료) | — | 200/503 | — |

위 API는 UI가 직접 호출하지 않는다. UI-facing 중간 백엔드가 `request_id`와 사용자 원문을 관리한 뒤 `/internal/recommendations`를 호출한다. 위험 사이렌 파이프라인은 아직 구현하지 않았으며 이 서비스에서 호출하지 않는다.
...
`region`, `industry_code`, `special_condition_text`는 중간 백엔드가 전달하는 값이다. FastAPI는 UI 입력을 재작성하지 않고 `RecommendationRequest`로 매핑해 `recommendation_pipeline.run_pipeline()`에 전달한다. `conditions`, `confirmation_required`, `unsupported_conditions`는 파이프라인 실행 결과의 `input_interpretation`/`summary`에서 반환된다.

분기(`RECOMMENDATION_QUARTER`), 데이터 소스(`RECOMMENDATION_SOURCE`), LLM 모드(`RECOMMENDATION_LLM_MODE`), 후보 수(`RECOMMENDATION_DEFAULT_LIMIT`)는 FastAPI 서버 환경변수로 관리하며 중간 백엔드 요청에는 포함하지 않는다.
...
{
  "request_id": "backend-request-123",
  "run_id": "20260903T120000Z-abc123",
  "status": "completed",
...
#### 결정 D-1: 추천 API 모듈러 모놀리스 (호스트 프로세스)

| | 선택 | 대안 |
| --- | --- | --- |
| 방식 | 중간 백엔드가 UI의 지역 체크박스·자연어 조건을 소유하고, EC2에서 실행 중인 `recommendation-api` FastAPI 프로세스가 전체 추천 파이프라인(C4~C8)을 한 프로세스 내 함수 호출로 실행한다. PostgreSQL만 Docker Compose의 `db` 컨테이너로 둔다. | 입력·엔진·RAG를 각각 별도 마이크로서비스·컨테이너로 배포 |
| 이유 | UI 입력 처리와 추천 실행의 책임은 나누되, 파이프라인 내부 단계까지 REST로 쪼개지 않는다. API 이미지 빌드·배포 경계를 추가하지 않고 DB 컨테이너만 유지해 현재 EC2 운영을 단순화한다. | 트래픽·팀이 커지거나 별도 스케일 요구가 생길 때만 특정 컴포넌트를 분리 |
| 되돌리는 비용 | **낮음**. `/internal/recommendations` 계약을 유지한 채 중간 백엔드와 추천 API의 배포 위치만 바꿀 수 있다. |
...
```mermaid
flowchart LR
    subgraph host[상시 가동 서버 1대 - EC2]
      RP[리버스 프록시<br/>TLS · 공개 진입점]
      MB[middle-backend<br/>UI 입력 처리]
      API[recommendation-api<br/>FastAPI + 전체 추천 파이프라인]
      DB[(Docker PostgreSQL<br/>PostGIS + pgvector)]
      DATA[(data · artifacts<br/>호스트 디렉토리)]
      OUT[(output<br/>실행 산출물)]
      RP --> MB
      MB -->|REST| API
      API -->|127.0.0.1:55432| DB
      API --> DATA
      API --> OUT
    end
    ...
    LLMX[LLM API]
    API -->|HTTPS + 예산가드| LLMX
    REG -->|승인 후 data/artifacts 교체| DATA
    OPS[운영자] -->|실행/승인| PIPE
```

- **앱**: EC2의 가상환경에서 `uvicorn api.main:app`으로 실행하며 `api/`와 `scripts/`의 전체 추천 파이프라인을 in-process로 호출한다. 현재 중간 백엔드는 별도 구현 전이며, 구현되면 FastAPI의 `/internal/recommendations`를 호출한다.
- **DB**: PostgreSQL만 `db` 컨테이너로 실행하고 API는 호스트 공개 주소 `127.0.0.1:55432`로 접근한다.
- **프록시**: 중간 백엔드가 공개 진입점이 되면 nginx/Caddy가 TLS·도메인·레이트리밋을 담당한다. recommendation-api 포트는 외부에 직접 노출하지 않는 것을 기본으로 한다.
- **배치**: 상시 실행 불필요(분기 1회) — 같은 서버의 온디맨드 잡(cron/수동) 또는 CI 러너. 앱 프로세스와 자원 분리(nice/cgroup) 권장.
- **데이터**: `data/`와 `artifacts/`는 EC2 호스트 디렉토리에서 읽고, 실행 결과는 `output/`에 기록한다. 승인된 데이터 버전으로 교체한 후 `/readyz`를 확인한다.
- **배포 절차**: EC2에서 Compose로 `db` 기동·health 확인 → 가상환경 의존성 설치 → `uvicorn`을 systemd 등 프로세스 관리자로 기동 → `/readyz` 확인. 무중단이 필요하면 인스턴스 2개 + 프록시 업스트림 전환(예산 확정 후 판단).
