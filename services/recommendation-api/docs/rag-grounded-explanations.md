...
설명 후처리는 질문 관련 출처에 연결되지 않은 선택적 이유·배경 문장을 제거한다. 템플릿 복귀에도 같은 정책을 적용하며 필수 경고·결측은 유지한다. 검증된 SQL 행은 후보의 상권 코드 또는 행정동 코드와 업종이 일치하는지 확인한 뒤 주제·공간단위별로 배경 문장을 직접 표시한다. 행정동 코드 연결은 후보 location.overlapping_units.admin_dong을 사용하며 이름은 표시 용도다. 서로 다른 상권·행정동 값은 충돌로 처리하거나 합산하지 않고 각각 인용한다. 출처·기간·단위·실제 조사권역·대리지표 한계를 포함하며 `retrieval-*`를 인용한다. 같은 주제에 복수 값/기간/권역이 있으면 단일 값을 임의 선택하지 않고 미확인 사유를 표시한다. 이 서버 렌더링은 모델의 재서술 의미 검토와 별개이며 추가 LLM 호출이 없다.
