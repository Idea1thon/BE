...
서울 전체 상권/행정동이 시간이 지나며 어떻게 분류가 이동했는지 추적한다.

scripts/songpa_change_trend_layer.py를 송파구 한정 없이 서울 전체로 일반화한 버전
...
def main():
    trdar_ci = load_change_indicator("상권_코드", "서울시 상권분석서비스(상권변화지표-상권).csv")
    dong_ci = load_change_indicator("행정동_코드", "서울시 상권분석서비스(상권변화지표-행정동).csv")

    quarters = sorted(trdar_ci["기준_년분기_코드"].unique())
    print(f"분석 분기: {quarters[0]} ~ {quarters[-1]} (총 {len(quarters)}개 분기)")
    print(f"서울 전체 상권 {trdar_ci['상권_코드'].nunique()}개 / 행정동 {dong_ci['행정동_코드'].nunique()}개\n")

    trdar_counts, trdar_share = quadrant_share_by_quarter(trdar_ci)
    dong_counts, dong_share = quadrant_share_by_quarter(dong_ci)

    trdar_counts.to_csv(os.path.join(SEOUL_DIR, "seoul_change_indicator_counts_trdar.csv"), encoding="utf-8-sig")
    dong_counts.to_csv(os.path.join(SEOUL_DIR, "seoul_change_indicator_counts_dong.csv"), encoding="utf-8-sig")
    trdar_share.to_csv(os.path.join(SEOUL_DIR, "seoul_change_indicator_trend_trdar.csv"), encoding="utf-8-sig")
    dong_share.to_csv(os.path.join(SEOUL_DIR, "seoul_change_indicator_trend_dong.csv"), encoding="utf-8-sig")
    ...
    plot_stacked_share(trdar_share, "서울 전체 상권: 분기별 상권변화지표 사분면 비중 추이 (2021Q1~2026Q1)",
                        os.path.join(FIG_DIR, "seoul_change_trend_trdar.png"))
    plot_stacked_share(dong_share, "서울 전체 행정동: 분기별 상권변화지표 사분면 비중 추이 (2021Q1~2026Q1)",
                        os.path.join(FIG_DIR, "seoul_change_trend_dong.png"))
    ...
    transition_csv = os.path.join(SEOUL_DIR, "seoul_change_indicator_transition_2021_to_2026.csv")
