...
def build_candidate(
    ...
    host = host_hits[0][1] if host_hits else None
...
def run_pipeline(
    ...
    if not re.fullmatch(r"[0-9]{5}", request.quarter):
        raise PipelineError(f"분기 코드는 5자리 숫자여야 합니다: {request.quarter}")
    ...
    (out_dir / "request.json").write_text(json.dumps(request_payload, ensure_ascii=False, indent=2), encoding="utf-8")
    (out_dir / "candidates.json").write_text(json.dumps(candidates, ensure_ascii=False, indent=2), encoding="utf-8")
    (out_dir / "explanations.json").write_text(json.dumps(explanations, ensure_ascii=False, indent=2), encoding="utf-8")
    (out_dir / "coverage-summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    (out_dir / "run-manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    notes = [
    ...
    (out_dir / "run-notes.md").write_text("\n".join(notes) + "\n", encoding="utf-8")
    if errors:
