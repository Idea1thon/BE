"""아파트(소득 대리지표) x 업종별 매출 상관관계 — 단일분기(20261) 대신 전체 공통분기 패널로 확장.

추정매출은 20251~20261(5개 분기)만 있고 아파트는 20211~20261(21개 분기)까지 있어,
두 데이터셋의 **공통 분기**(20251, 20252, 20253, 20254, 20261)만 사용한다. 분기가 다른
데이터끼리 조인하면 안 되므로, 각 상권-분기 쌍을 하나의 관측치로 보는 패널 구조로 결합한다.

특히 호프-간이주점(주점)은 이전 단일분기(20261) 분석에서 상관계수가 0.01로 거의
무관하게 나왔는데, 패널로 확장해도 그 결과가 유지되는지 확인한다.
"""
import os
...
def load_apartment_panel(trdar_codes, quarters):
    path = os.path.join(DATA, "아파트", "서울시 상권분석서비스(아파트-상권).csv")
    df = pd.read_csv(path, encoding="cp949")
    df = df[df["기준_년분기_코드"].isin(quarters) & df["상권_코드"].isin(trdar_codes)]
    return df[["기준_년분기_코드", "상권_코드", "상권_코드_명", "아파트_평균_면적", "아파트_평균_시가"]]


def load_sales_panel(trdar_codes, quarters):
    path = os.path.join(DATA, "추정매출", "2026", "서울시 상권분석서비스(추정매출-상권).csv")
    df = pd.read_csv(path, encoding="cp949")
    df = df[
        df["기준_년분기_코드"].isin(quarters)
...
def main():
    ...
    apt = load_apartment_panel(trdar_codes, quarters)
    sales = load_sales_panel(trdar_codes, quarters)
    ...
    out_csv = os.path.join(SONGPA_DIR, "songpa_apartment_sales_panel.csv")
    ...
    compare_csv = os.path.join(SONGPA_DIR, "songpa_apartment_sales_pooled_vs_single.csv")
    ...
    trend_csv = os.path.join(SONGPA_DIR, "songpa_apartment_sales_quarterly_corr_trend.csv")
    ...
    ax.set_title(f"송파구: 아파트 x 업종별 매출 상관계수 (전체분기 pooled, n={len(panel)})",
                 color=INK_PRIMARY, fontsize=11, pad=12)
    cbar = fig.colorbar(im, ax=ax, shrink=0.8)
    cbar.ax.tick_params(colors=INK_MUTED)
    cbar.outline.set_visible(False)
    fig.tight_layout()
    out1 = os.path.join(FIG_DIR, "songpa_apartment_sales_corr_heatmap_pooled.png")
    ...
    ax.set_title("송파구: 분기별 아파트-매출 상관계수 추이 (회색=나머지 9개 업종, 주황=호프-간이주점)",
    ...
    fig.tight_layout()
    out2 = os.path.join(FIG_DIR, "songpa_apartment_sales_quarterly_trend.png")
