"""아파트(소득 대리지표) x 업종별 매출 상관관계 — 상권만이 아니라 상권배후지·행정동까지
겹침 크로스워크로 연결해서 비교.

두 갈래로 분석한다:

(A) 세 단위를 각각 독립적으로 비교: 상권 자체 아파트시가 vs 상권 매출, 상권배후지 자체
    아파트시가 vs 상권배후지 매출, 행정동 자체 아파트시가 vs 행정동 매출. 단위마다
    표본(상권 수/배후지 수/행정동 수)과 소득 분포가 달라 상관계수가 어떻게 달라지는지
    비교한다.

(B) 상권 기준으로 "겹치는 영역을 결합"한 소득지표를 만들어 비교: 상권 자체 아파트시가
    대신, 그 상권이 걸친 행정동들의 아파트시가를 면적비율로 가중평균한 값, 그리고 그
    상권의 배후지 + 그 배후지와 겹치는 인접 배후지들의 아파트시가를 겹침면적으로
    가중평균한 값을 만들어, 상권 매출과의 상관관계가 "상권 자체 값"보다 강해지는지
    확인한다. (프로젝트 원칙: 겹침은 분할이 아니라 결합 대상 — CLAUDE.md/PROGRESS.md 참고)
...
def load_codes():
    trdar = set(pd.read_csv(os.path.join(SONGPA_DIR, "songpa_trdar_membership.csv"),
                             encoding="utf-8-sig")["TRDAR_CD"].astype(int))
    alley = set(pd.read_csv(os.path.join(SONGPA_DIR, "songpa_alley_membership.csv"),
                             encoding="utf-8-sig")["ALLEY_TRDA"].astype(int))
    dong_df = pd.read_csv(os.path.join(DATA, "영역", "행정동", "서울시 상권분석서비스(영역-행정동).csv"), encoding="cp949")
    dong = set(dong_df.loc[dong_df["행정동_코드"].astype(str).str.startswith("11710"), "행정동_코드"].astype(int))
...
def build_combined_income_by_trdar(trdar_codes, quarters):
    ...
    for trdar in trdar_codes:
        dong_weights = tw[tw["TRDAR_CD"] == trdar][["ADSTRD_CD", "ratio_of_trdar"]].values.tolist()
        alley_row = ta[ta["TRDAR_CD"] == trdar]
        own_alley = int(alley_row["ALLEY_TRDA"].iloc[0]) if len(alley_row) else None

        if own_alley is not None:
            neigh = aso[(aso["ALLEY_TRDA_A"] == own_alley) | (aso["ALLEY_TRDA_B"] == own_alley)]
            alley_weights = [(own_alley, 1.0)]  # 자기 자신은 기준 가중치 1.0
            for _, r in neigh.iterrows():
                ...
                alley_weights.append((other, float(ratio)))
        else:
            alley_weights = []
...
def main():
    ...
    print(f"송파구 상권 {len(trdar_codes)}개 / 배후지 {len(alley_codes)}개 / 행정동 {len(dong_codes)}개\n")
    ...
    compare_csv = os.path.join(SONGPA_DIR, "songpa_apartment_sales_by_unit.csv")
    ...
    ax.set_title("송파구: 단위별(상권/배후지/행정동) 아파트시가-매출 상관계수", color=INK_PRIMARY, fontsize=12, pad=12)
    cbar = fig.colorbar(im, ax=ax, shrink=0.8)
    cbar.ax.tick_params(colors=INK_MUTED)
    cbar.outline.set_visible(False)
    fig.tight_layout()
    out1 = os.path.join(FIG_DIR, "songpa_apartment_sales_by_unit.png")
    ...
    combined_csv = os.path.join(SONGPA_DIR, "songpa_combined_income_by_trdar.csv")
    ...
    combo_csv = os.path.join(SONGPA_DIR, "songpa_apartment_sales_combined_vs_own.csv")
    ...
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.set_title("송파구: 상권 자체 vs 겹침결합 소득지표의 매출 설명력 비교", color=INK_PRIMARY, fontsize=11, pad=12)
    cbar = fig.colorbar(im, ax=ax, shrink=0.8)
    cbar.ax.tick_params(colors=INK_MUTED)
    cbar.outline.set_visible(False)
    fig.tight_layout()
    out2 = os.path.join(FIG_DIR, "songpa_apartment_sales_combined_vs_own.png")
